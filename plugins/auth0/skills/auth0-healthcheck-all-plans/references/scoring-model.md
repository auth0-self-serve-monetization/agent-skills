...
- **Passing-count fallback.** The formula needs `#passing`. Verify the report shape against a fresh run — current CheckMate emits a **flat array of finding objects** (each with `severity`, `status`, `title`, `severity_message`), not a `data.report.summary[]` wrapper. If an explicit passing count isn't labeled, derive the denominator only from the **real** number of checks the parser actually returned: `weighted_total = weighted_failures + (total_checks − #failures)`, where `total_checks` is the count of entries in the parsed report. **Never estimate `total_checks` from a hardcoded or "documented" figure** (e.g. an assumed ~50-check total) — that drifts as CheckMate changes and produces a misleading score. If neither a real failures-with-total nor a passing count can be established from the parsed report, fall back to **"Not scored"** rather than guessing.
