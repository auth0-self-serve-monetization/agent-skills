...
This skill **orchestrates** two sister skills rather than reimplementing them: `auth0-checkmate` (tenant scan + findings) and `account-enrichment` (company/use-case intelligence). It does not modify `auth0-launch-readiness` or `auth0-plan-fit-diagnostic`.
...
**Source-of-truth rules (non-negotiable):** all pricing / MAU limits / feature availability come from `pricing.md` — never hardcode them. `tenant_domain` always comes from the Auth0 CLI, never from enrichment. Never quote or estimate a price for Enterprise.

**Enrichment is internal context, not customer-facing output.** The customer is self-auditing their own tenant, so do NOT dump the enrichment profile back at them. Surface only: (a) the derived **use-case classification**, (b) the customer's **product names** woven into recommendations, and (c) the **provenance / confidence note**. **Never display raw enrichment fields to the customer** — fit scores (`a4aa_fit_score`, `auth0_fit_score`), firmographics (funding, valuation, employee count, investors, public status), or the internal `auth0_plan_recommendation` field. These are internal qualification signals (account-enrichment was built for Auth0 outreach), they're things the customer already knows about themselves, and showing them erodes trust. Exception: the **Talk-to-Sales brief** may include company facts — it is addressed to sales, not shown as the customer's result.
...
3. **Enrichment (company/use-case):** if provided, use it; otherwise **invoke the `account-enrichment` sister skill** with the company domain (ask for the domain only if unknown). Record **provenance** (live research vs. training-knowledge fallback) and `confidence_score`.
...
3. **Plan matching** keyed on current plan — [references/plan-matching-logic.md](references/plan-matching-logic.md):
   - **Self-service (no Enterprise need):** recommend a specific plan + **exact cost** from `pricing.md`. Suggest the **A4AA add-on only when `a4aa_fit_score ≥ 0.4` AND there are concrete agent integrations** (`integrations.length > 0` / autonomous-action workflows in enrichment) — a high fit score alone, with no integrations, is NOT enough (per `plan-matching-logic.md`).
...
- **Missing / low-confidence enrichment:** run a technical-only assessment; ask `business_model` if null; add *"Company intelligence unavailable; recommendation based on technical posture alone."* If enrichment came from a training-knowledge fallback or `confidence_score < 0.5`, treat company specifics as approximate, **downgrade an inferred (Class B) Enterprise recommendation to SOFT**, and add *"Company details are drawn from general knowledge and may be out of date; verify specifics before sharing externally."*
...
- `tenant_domain` is always the CLI value, never derived from enrichment or the company domain.
...
- Don't reinvent CheckMate/enrichment — invoke them.
